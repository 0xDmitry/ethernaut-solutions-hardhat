...
contract StatisticsLevelSuperseder is Initializable {
    ...
    event playerScoreProfile(
        ...
        uint256 indexed averageCompletionTime,
        uint256 indexed globalLevelsCompleted
    );
    ...
    function getTotalNoOfLevelInstancesCreatedByPlayer(
        address player
    ) public view playerExistsCheck(player) returns (uint256) {
        return globalNoOfInstancesCreatedByPlayer[player];
    }

    // number of levels completed by player
    function getTotalNoOfLevelInstancesCompletedByPlayer(
        address player
    ) public view playerExistsCheck(player) returns (uint256) {
    ...
    function getTotalNoOfFailedSubmissionsByPlayer(
        address player
    ) public view playerExistsCheck(player) returns (uint256) {
        return globalNoOfFailedSubmissionsByPlayer[player];
    }

    function getTotalNoOfLevelsCompletedByPlayer(
        address player
    ) public view playerExistsCheck(player) returns (uint256) {
    ...
    function getTotalNoOfFailuresForLevelAndPlayer(
        address level,
        address player
    ) public view playerExistsCheck(player) levelExistsCheck(level) returns (uint256) {
        return
    ...
    function isLevelCompleted(
        address player,
        address level
    ) public view playerExistsCheck(player) levelExistsCheck(level) returns (bool) {
        return playerStats[player][level].isCompleted;
    }

    // How much time a player took to complete a level (in seconds)
    function getTimeElapsedForCompletionOfLevel(
        address player,
        address level
    ) public view playerExistsCheck(player) levelExistsCheck(level) returns (uint256) {
    ...
    function getPercentageOfLevelsCompleted(
        address player
    ) public view playerExistsCheck(player) returns (uint256) {
    ...
    function updateAverageTimeTakenToCompleteLevelsByPlayer(
        ...
        if (averageTimeTakenToCompleteLevels[player] == 0) {
            ...
            newAverageTimeTakenToCompleteLevels =
                ((lastAverageTime * (totalNoOfLevelsCompletedByPlayer - 1)) +
                    timeTakenForThisSuccessfulSubmission) /
    ...
    function getNoOfFailedSubmissionsForLevel(
        address level
    ) public view levelExistsCheck(level) returns (uint256) {
        return levelStats[level].noOfSubmissions_Failed;
    }

    function getNoOfInstancesForLevel(
        address level
    ) public view levelExistsCheck(level) returns (uint256) {
        return levelStats[level].noOfInstancesCreated;
    }

    function getNoOfCompletedSubmissionsForLevel(
        address level
    ) public view levelExistsCheck(level) returns (uint256) {
    ...
    function setSubstitutionAddresses(
        address _oldLevelContractAddress,
        address _newLevelContractAddress
    ) public onlyOperator {
        require(dumpStage == DumpStage.SET_ADDRESSES, "Not correct dump stage");
        require(
            levelExists[_oldLevelContractAddress],
            "Address to be superseded is not an existing level"
        );
        ...
        require(
            _oldLevelContractAddress != levels[levels.length - 1],
            "Old address can't be last level address"
        );
    ...
    function dumpLevelFirstInstanceCreationTime() public onlyOperator {
        ...
        do {
            timestamp = levelFirstInstanceCreationTime[players[_usersArrayIndex]][
                _oldLevelContractAddress
            ];

            if (timestamp > 0) {
                levelFirstInstanceCreationTime[players[_usersArrayIndex]][
    ...
    function dumpLevelFirstCompletionTime() public onlyOperator {
        ...
        do {
            timestamp = levelFirstCompletionTime[players[_usersArrayIndex]][
                _oldLevelContractAddress
            ];

            if (timestamp > 0) {
                levelFirstCompletionTime[players[_usersArrayIndex]][
                    _newLevelContractAddress
                ] = timestamp;
    ...
    function dumpPlayerStats() public onlyOperator {
        ...
        do {
            ...
            if (actualInstance.instance != address(0)) {
                playerStats[players[_usersArrayIndex]][_newLevelContractAddress]
                    .instance = actualInstance.instance;
                playerStats[players[_usersArrayIndex]][_newLevelContractAddress]
                    .isCompleted = actualInstance.isCompleted;
                playerStats[players[_usersArrayIndex]][_newLevelContractAddress]
                    .timeCreated = actualInstance.timeCreated;
                playerStats[players[_usersArrayIndex]][_newLevelContractAddress]
                    .timeCompleted = actualInstance.timeCompleted;
                playerStats[players[_usersArrayIndex]][_newLevelContractAddress]
                    .timeSubmitted = actualInstance.timeSubmitted;

                playerStats[players[_usersArrayIndex]][
                    _oldLevelContractAddress
                ] = levelInstanceEmpty;
            }
    ...
    function getLevelFirstInstanceCreationTime(
        address player,
        address level
    ...
    function getLevelFirstCompletionTime(
        address player,
        address level
    ) public view returns (uint256) {
    ...
    function getPlayerStats(
        address player,
        address level
    ) public view returns (LevelInstance memory) {
